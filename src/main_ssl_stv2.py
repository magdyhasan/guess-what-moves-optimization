...
def train_seq(args, it, val_seq):
    ...
    train_loader, val_loader = config.loaders(cfg, val_seq)

    # overfit single batch for debug
    # sample = next(iter(loader))

    criterions = {
        'ssl_consistency': (losses.SSLConsistencyLoss(cfg, model), cfg.GWM.LOSS_MULT.REC, lambda x: 1)}
    ...
    with torch.autograd.set_detect_anomaly(cfg.DEBUG) and \
        ...
        while iteration < total_iter + it:
            # list of list of dict
            for sample in train_loader:
                if iteration > total_iter + it:
                    break
                ...
                if (iteration + 1) % 2 == 0 or (iteration + 1) in [1, 50, 500]:
                    model.eval()
                    ...
                    if iou := eval_unsupmf(cfg=cfg, val_loader=val_loader, model=model, criterion=criterion,
                        ...
                        if cfg.SOLVER.CHECKPOINT_PERIOD and iou > iou_best:
                            ...
                            # eval_out(cfg=cfg, val_loader=val_loader, model=model, criterion=criterion,
                            #                writer=None, writer_iteration=iteration + 1, use_wandb=cfg.WANDB.ENABLE)
                        wandb.log({'eval/IoU/' + val_seq[0]: iou}, step=iteration + 1)
                        if cfg.WANDB.ENABLE:
...
def main(args):
    ...
    for seq in val_seq:
        ...
        ious, biou = train_seq(args, 100, [seq])
        seqs_ious[seq] = ious
        biou_sum += biou
        print('{}: {}', seq, biou)
    print('best_sum_{}, best_mean_{}'.format(biou_sum, biou_sum/(len(val_seq) * 1.0)))
    for _, seq_ious in seqs_ious.items():
        for it, it_res in seq_ious.items():
            print('at it {}'.format(it))
            iou_sum = 0
            for seq in val_seq:
                print('seq {} result: {}'.format(seq, seqs_ious[seq][it]))
                iou_sum += seqs_ious[seq][it]
            print('it_{}: mean_{}'.format(it, iou_sum/(len(val_seq)) * 1.0))
        break
