...
class FlowSSLVIDDetectron(Dataset):
    ...
    def __len__(self):
        return len(self.samples) - self.frames_num + 1

    def __getitem__(self, idx):
        dataset_dicts = []
        for f_idx in np.arange(idx, idx + self.frames_num):
            dataset_dict = {}
            flow_dir = Path(self.data_dir[0]) / self.samples[f_idx]
            fid = self.samples_fid[str(flow_dir.parent)][flow_dir]
            
            flow_dir = Path('/storage/user/mahmo/temp/ssl-vos/data/SegTrackv2/flow_1_arflow_cityscapes_percentile_90') / self.samples[f_idx].replace('.flo', '.npy')
            r_flow_dir = Path('/storage/user/mahmo/temp/ssl-vos/data/SegTrackv2/flow_reverse_1_arflow_cityscapes_percentile_90') / self.r_samples[f_idx].replace('.flo', '.npy')
            

            flo = np.load(flow_dir)
            flo = np.stack(rescale_flow(flo, scale_factor=self.resolution, renormalize=False)).transpose(1, 2, 0)
            flo = torch.tensor(flo, dtype=torch.float)
            
            r_flo = np.load(r_flow_dir)
            r_flo = np.stack(rescale_flow(r_flo, scale_factor=self.resolution, renormalize=False)).transpose(1, 2, 0)
            r_flo = torch.tensor(r_flo, dtype=torch.float)
            
            dataset_dict["gap"] = 'gap1'

            suffix = '.jpg'
            rgb_dir = (self.data_dir[1] / self.samples[f_idx]).with_suffix(suffix)
            gt_dir = (self.data_dir[2] / self.samples[f_idx]).with_suffix('.png')
            ...
            gwm_dir = (Path(str(self.data_dir[2]).replace('Annotations', 'gwm')) / self.samples[f_idx]).with_suffix(
                '.png')
            gwm_seg_gt = None
